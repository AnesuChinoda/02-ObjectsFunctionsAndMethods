...
Authors: David Mutchler, Vibha Alangar, Mark Hays, Amanda Stouder,
         their colleagues and Anesu Chinoda.
...
def main():
    ...
    #better_draw_circles(rg.Point(50, 50), 30)
    #better_draw_circles(rg.Point(25, 25), 3)
    p = rg.Point(15, 25)
    even_better_draw_circles(p, 10, 6, 'red', 5)
...
def better_draw_circles(point,Increasing_Circles):
    ...
    for k in range(1, 11):  # k becomes 1, 2, 3, ... 10
        ...
        # Go DOWN 15 pixels, ending up pointing east again
        turtle.right(90)
        turtle.forward(Increasing_Circles)
        turtle.left(90)

        turtle.pen_down()
        turtle.draw_circle(Increasing_Circles * k)  # Radius 15, 30, 45, 60, ...
...
def even_better_draw_circles(Center_Of_Circle, Increasing_Circles, Number_Of_Circles, Color_Of_Circle, Thickness_Of_Circle):
    ...
    turtle = rg.SimpleTurtle()
    turtle.pen = rg.Pen(Color_Of_Circle, Thickness_Of_Circle)
    turtle.pen_up()
    turtle.go_to(Center_Of_Circle)
    print(Center_Of_Circle)
    turtle.set_heading(0)  # Point "east" (towards the right)
